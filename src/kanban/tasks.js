...
TaskList.prototype = {
   ...
   */
  get(id) {
    return this.issues[this._search(id)]
  },
  ...
  for (var obj of group) {
    ...
    for (var propName in obj) {
          ...
          if (newobj[propName] === null) {
            throw new Error(`DOM object ${propName} not found
              Bad query ${obj[propName]}`)
          }
    }
    objs.push(newobj)
  }
...
ChainController.prototype = {
   ...
   */
  _rSelect(obj, list) {
    let html = '<option selected value="">select task</option>\n'
    for (var elem of list) {
